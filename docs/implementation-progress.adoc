...
|⬜
|Create `src/main/pascal/PasBuild.lpr` entry point
|Main program file
...
|⬜
|Parse `<build>` section
|`mainSource`, `outputDirectory`, `executableName`
...
|⬜
|Implement `ParseArguments(out Goal: string; out Profile: string): Boolean`
|Extract goal and `-p` flag
...
|Show PasBuild + detected FPC version
...
|⬜
|Add logging helper: `LogInfo(Message: string)`
|Format: `[INFO] Message`

|⬜
|Add logging helper: `LogError(Message: string)`
|Format: `[ERROR] Message`, write to stderr
|===
...
|Automatically run: clean → compile → package
|===

...
Many applications need non-code resources bundled with the executable. This goal would automate copying resource files to the output directory and including them in release packages.

=== Version Injection

**Goal:** Automatic version define

**Behavior:**

Automatically add compiler define: `-dVERSION='1.0.0'` based on `<version>` from project.xml
...
Eliminates manual version synchronization between project.xml and source code. Ensures version displayed by application always matches build configuration.

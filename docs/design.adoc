...
4. Save archive to project root
...
* Multiple `-p` flags → All profiles merged (future enhancement)
...
**Future Enhancement:** Global `<build><compilerOptions>` that apply to all profiles
...
[source,bash]
----
pasbuild compile -p debug -p logging -p linux-gtk
----

**Semantics:** Merge all profile defines
